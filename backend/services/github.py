...
async def fetch_repo_info(url: str) -> dict:
    ...
    async with httpx.AsyncClient(timeout=30) as client:
        ...
        contents_resp = await client.get(f'{GITHUB_API}/repos/{owner}/{repo}/contents', headers=HEADERS)
        file_tree = _parse_tree(contents_resp.json()) if contents_resp.status_code == 200 else []

        readme_text = ''
        readme_resp = await client.get(f'{GITHUB_API}/repos/{owner}/{repo}/readme', headers=HEADERS, params={'ref': repo_data.get('default_branch', 'main')})
...
def _parse_tree(items: list[dict]) -> list[dict]:
    tree: list[dict] = []
